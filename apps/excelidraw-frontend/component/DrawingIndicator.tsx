...
export function DrawingIndicator({ roomId, socket }: DrawingIndicatorProps) {
    ...
    const handleMessage = (event: MessageEvent) => {
      try {
        ...
        if (
          ...
          setTimeout(() => {
            setActiveDrawers((prev) =>
              prev.filter((d) => d.userId !== newDrawer.userId),
            );
        ...
        if (data.type === "user_activity_update" && data.roomId === roomId) {
          if (data.activity === "started_drawing") {
            ...
            setActiveDrawers((prev) =>
              prev.filter((d) => d.userId !== data.userId),
            );
            if (activeDrawers.length <= 1) {
              setIsVisible(false);
            }
  ...
  return (
    <div className="fixed top-48 right-4 z-50 bg-white/95 backdrop-blur-md rounded-xl p-4 border border-gray-200/60 shadow-lg max-w-xs">
      <div className="flex items-center space-x-2 mb-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">
            {activeDrawers.length} user{activeDrawers.length > 1 ? "s" : ""}{" "}
            drawing
          </p>
        </div>
      </div>
